...
这一节记录调参过程中实测才发现的问题，写下来免得别人再趟一遍。

重建最好的那版（14×14，recon 0.0060）生成的数字反而最差。折腾了几轮才想明白：重建看的是每个 token 能带多少信息，生成还要看索引分布规不规范，两件事的瓶颈不一样。14×14 版为了整除用了 stride-3 下采样，卷积窗口完全不重叠，数字 3 的弧线被混叠拍成了 8；stride-2 窗口有重叠就没这个问题，但 28 做两次 stride-2 只能到 7，这就是隐空间定在 7×7 的原因。

码字维度 D 从 64 砍到 16 之后，同数重建基本持平，利用率还上去了。之前一直以为容量跟着 D 走，实际瓶颈在 token 数乘以 log K，D 只决定码字向量的表达空间。这也解释了为什么早期把 K 加到 128、D 加到 64 没带来多少收益。

死码问题卡了最久。按论文惯例用 uniform(-1/K, 1/K) 初始化码本，encoder 输出却有 ±25/维 的量级，差了三个数量级，开局几乎所有样本都 argmin 到同一小撮码字，剩下的码字梯度恒为零，直接死透。14×14 版有 32 个码字从头到尾停在初始值附近。EMA 版能活下来是运气好：它把码字直接赋值成 z_e 的均值，不受学习率限制，一步就跨过了尺度差。真正修掉这个问题的办法很土--拿第一个 batch 的 encoder 输出当码本初值，改完首步 vq_loss 从 611 掉到 0.023，之后再没见过死码。

活码数和 perplexity 利用率不是一回事，前一个管有没有码字死掉，后一个管用得均不均匀。空白背景块天然比罕见转角多，MNIST 的笔画模式出现频率本来就不均等，所以利用率到不了 100% 是正常的，硬追这个数字等于要求真实数据服从均匀分布。

换 Gated PixelCNN 之前先怀疑过朴素掩码卷积的盲区，拿梯度依赖图实测了一把：7×7 上盲点其实是 0（首层 7×7 大核的感受野已经盖满全图），28×28 上才严重（783 个上文位置里 704 个是盲的）。所以在这个尺度上换 Gated 的真正收益不在消盲点，而在每层都能注入类别条件。早期版本只在第一层注入一次，类别信息传到深层就稀释了，让它画 2 会画出 Q。
